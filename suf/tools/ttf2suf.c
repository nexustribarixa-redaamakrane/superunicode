/**
 * @file ttf2suf.c
 * @brief TrueType (.ttf) to SuperUnicode Font (.suf) CLI Converter
 */
...
int main(int argc, char **argv) {
    if (argc < 3) {
        fprintf(stderr, "Usage: %s <input.ttf> <output.suf>\n", argv[0]);
        return 1;
    }
    ...
    suf_status_t st = suf_conv_ttf_to_suf(in_buf, (size_t)sz, &builder);
    free(in_buf);

    if (st != SUF_OK || !builder) {
        fprintf(stderr, "Error: TTF to SUF conversion failed with code %d\n", st);
        return 1;
    }
    ...
    printf("[SUCCESS] Converted TrueType '%s' -> SUF '%s'\n", in_path, out_path);
    return 0;
}
