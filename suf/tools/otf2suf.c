/**
 * @file otf2suf.c
 * @brief OpenType CFF (.otf) to SuperUnicode Font (.suf) CLI Converter
 */
...
int main(int argc, char **argv) {
    if (argc < 3) {
        fprintf(stderr, "Usage: %s <input.otf> <output.suf>\n", argv[0]);
        return 1;
    }
    ...
    suf_status_t st = suf_conv_otf_to_suf(in_buf, (size_t)sz, &builder);
    free(in_buf);

    if (st != SUF_OK || !builder) {
        fprintf(stderr, "Error: OTF to SUF conversion failed with code %d\n", st);
        return 1;
    }
    ...
    printf("[SUCCESS] Converted OpenType '%s' -> SUF '%s'\n", in_path, out_path);
    return 0;
}
